...
const JobListings = () => {
  return (
    <div className="w-full h-auto flex flex-col gap-[50px] p-7 lg:px-20 lg:pb-10 bg-[#F9F9FF]">
      {jobs.length === 0 ? (
        <p className="text-3xl font-bold text-[#16205F] mb-[30px]">
        ...
        jobs.map((job) => (
          <div key={job.category} className="w-full h-auto text-[#16205F]">
            ...
            {job.listings.map((listing, index) => (
              <div
                key={index}
                className="bg-white p-5 rounded-[4px] border-[2px] mb-[30px]"
              >
                <div className="flex flex-col lg:flex-row">
                  <div className="flex flex-col gap-[20px] w-full lg:w-[50%] mb-[50px]">
                    ...
                    <p className="font-semibold text-[28px] mb-1 lg:pr-[50px]">
                      {listing.role}
                    </p>
                    <p className="text-[#2D3748] text-[16px] font-normal lg:pr-[50px]">
                      {listing.description}
                    </p>
                  </div>
                  <div className="flex flex-col gap-[20px] w-full lg:w-[40%] mb-[50px]">
                    ...
                    <div className="flex gap-[12px] flex-wrap lg:pr-[30px]">
                      {listing.skills.map((skill, index) => (
                        <div
                          key={index}
                          className="w-auto px-3 py-1 rounded-[7px] bg-[#2D7EFF38] font-semibold text-[13px] flex items-center justify-center"
                        >
                          {skill}
                        </div>
                      ))}
                ...
                <Link
                  href={`/career/${listing.role
                    .toLowerCase()
                    .replace(/\s+/g, "-")}`}
                  className="flex bg-[#2D7EFF] text-white w-fit p-3 items-center gap-[10px] text-[14px] lg:mt-[30px] rounded-[7px]"
                >
                  APPLY NOW
                  <Image
